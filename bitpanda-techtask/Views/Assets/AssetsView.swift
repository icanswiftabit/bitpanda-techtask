...
final class AssetsView: UIView {    
    @Published private(set) var selectedSegment: AssetType = .cryptos
...
private extension AssetsView {
    func setUpBindings() {
        segmentControl.publisher(for: \.selectedSegmentIndex).sink { [weak self] tab in
            guard let selectedAssetType = AssetType(rawValue: tab) else { assert(false, "SelectedAssetType should be created") }
